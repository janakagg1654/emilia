...
Module({pattern: 'song ?(.*)', fromMe: sourav, desc: Lang.SONG_DESC}, (async (message, match) => { 
    ...
    if(sr.length < 1) return await message.sendReply(Lang.NO_RESULT);
        ...
        rows: [{title: sr[0].title, description: '', rowId:"song;"+sr[0].videoId+';'+myid},{title: sr[1].title, description: '', rowId:"song;"+sr[1].videoId+';'+myid},{title: sr[2].title, description: '', rowId:"song;"+sr[2].videoId+';'+myid},{title: sr[3].title, description: '', rowId:"song;"+sr[3].videoId+';'+myid},{title: sr[4].title, description: '', rowId:"song;"+sr[4].videoId+';'+myid},{title: sr[5].title, description: '', rowId:"song;"+sr[5].videoId+';'+myid},{title: sr[6].title, description: '', rowId:"song;"+sr[6].videoId+';'+myid},{title: sr[7].title, description: '', rowId:"song;"+sr[7].videoId+';'+myid},{title: sr[8].title, description: '', rowId:"song;"+sr[8].videoId+';'+myid},{title: sr[9].title, description: '', rowId:"song;"+sr[9].videoId+';'+myid},]} ]
        const listMessage = {
...
Module({pattern: 'yts ?(.*)', fromMe: sourav, desc: "Select and download songs from yt (list)"}, (async (message, match) => { 
    if (!match[1]) return message.sendReply("*Need words*")
            ...
            var videos = [];
            for ( var index=0; index<sr.length; index++ ) {videos.push({title: sr[index].title, description: '', rowId:"ytsl;"+sr[index].videoId+';'+myid});}
      const sections = [{
...
if (message.button && message.button.startsWith("ytsv") && message.button.includes(message.client.user.id.split("@")[0].split(":")[0])) {
    ...
    try { var yt = ytdl(message.button.split(";")[2], {filter: format => format.container === 'mp4' && ['720p', '480p', '360p', '240p', '144p'].map(() => true)}); } catch {
        var {url,thumbnail,title} = await ytdlServer("https://youtu.be/"+message.button.split(";")[2]);
        return await message.client.sendMessage(message.jid,{video: {url: url},mimetype: "video/mp4" , caption:title, thumbnail: await skbuffer(thumbnail)});}
...
if (message.button && message.button.startsWith("ytsa") && message.button.includes(message.client.user.id.split("@")[0].split(":")[0])) {
    ...
    var {details} = await getVideo(message.button.split(";")[2]);
    var thumb = await skbuffer(details.thumbnail.url);
    ffmpeg(stream)
        .audioBitrate(128)
        .save('./temp/song.mp3')
...
if (message.list && message.list.startsWith("song") && message.list.includes(message.client.user.id.split("@")[0].split(":")[0])) {
    try { var stream = ytdl(message.list.split(";")[1], {quality: 'highestaudio',}); } catch {
        var {url} = await ytdlServer("https://youtu.be/"+message.list.split(";")[1],"128kbps","audio"); 
        return await message.client.sendMessage(message.jid,{audio: {url: url},mimetype: 'audio/mpeg'}, {quoted: message.data});
    }
...
if (message.list && message.list.startsWith("ytv") && message.list.includes(message.client.user.id.split("@")[0].split(":")[0])) {
    try { var stream = ytdl(message.list.split(";")[1], {quality: 'highestaudio',}); } catch {
        var {url} = await ytdlServer("https://youtu.be/"+message.list.split(";")[1]); 
        return await message.client.sendMessage(message.jid,{audio: {url: url},mimetype: 'audio/mpeg'}, {quoted: message.data});
    }
    var {details} = await getVideo(message.list.split(";")[1]);
    var thumb = await skbuffer(details.thumbnail.url);
